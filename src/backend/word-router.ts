import type { Word, WordPractice, WordPracticeCompletion } from './database/database-models'
import { type ProcedureParams, withDatabase } from './procedure-utils'

// Word関連のprocedure（Worker外で実行可能）
async function getWordsByTopic({
  database,
  input,
}: ProcedureParams & { input: { topicId: number } }): Promise<Word[]> {
  const result = await database.executeQuery('SELECT * FROM Word WHERE topicId = ? ORDER BY id', [
    input.topicId,
  ])
...
async function getWord({
  database,
  input,
}: ProcedureParams & { input: { id: number } }): Promise<Word | null> {
...
async function createWord({
  database,
  input,
}: ProcedureParams & { input: { topicId: number; text: string; reading: string } }): Promise<Word> {
  await database.executeQuery('INSERT INTO Word (topicId, text, reading) VALUES (?, ?, ?)', [
    input.topicId,
    input.text,
    input.reading,
  ])
...
async function createWordPractice({
  database,
  input,
}: ProcedureParams & { input: { wordId: number; keystrokeTimes: string } }): Promise<WordPractice> {
...
async function createWordPracticeCompletion({
  ...
  const result = await database.executeQuery(
    'SELECT * FROM WordPracticeCompletion WHERE id = last_insert_rowid()'
  )
